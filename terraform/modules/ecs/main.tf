...
resource "aws_ecs_task_definition" "prashanth_task9_task" {
  family                   = "prashanth-task9-strapi-task"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      name  = "prashanth-task9-strapi-container"
...
resource "aws_ecs_service" "prashanth_task9_service" {
  name            = "prashanth-task9-strapi-service"
  cluster         = aws_ecs_cluster.prashanth_task9_cluster.id
  task_definition = aws_ecs_task_definition.prashanth_task9_task.arn
  desired_count   = 1  
